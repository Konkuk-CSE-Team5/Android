package com.konkuk.hackathon.core.data.repositoryimpl

import com.konkuk.hackathon.core.data.repository.VolunteerRepository
import com.konkuk.hackathon.core.network.request.UpdateRecordRequest
import com.konkuk.hackathon.core.network.response.VolunteerRecordDetailResponse
import com.konkuk.hackathon.core.network.response.VolunteerRecordUpdateFormResponse
import com.konkuk.hackathon.core.network.response.VolunteerRecordsResponse
import com.konkuk.hackathon.core.network.response.base.handleBaseResponse
import com.konkuk.hackathon.core.network.service.VolunteerService
import javax.inject.Inject

class VolunteerRepositoryImpl @Inject constructor(
    private val volunteerService: VolunteerService,
) : VolunteerRepository {

    override suspend fun getVolunteerRecords(): Result<VolunteerRecordsResponse> = runCatching {
        return volunteerService.getVolunteerRecords().handleBaseResponse()
    }
    
    override suspend fun getVolunteerRecord(recordId: Long): Result<VolunteerRecordDetailResponse> = runCatching {
        return volunteerService.getVolunteerRecord(recordId).handleBaseResponse()
    }
    
    override suspend fun getVolunteerRecordUpdateForm(recordId: Long): Result<VolunteerRecordUpdateFormResponse> = runCatching {
        return volunteerService.getVolunteerRecordUpdateForm(recordId).handleBaseResponse()
    }
    
    override suspend fun updateVolunteerRecord(recordId: Long, request: UpdateRecordRequest): Result<Unit> = runCatching {
        return volunteerService.updateVolunteerRecord(recordId, request).handleBaseResponse()
    }
}