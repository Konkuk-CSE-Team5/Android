...
fun phoneOnlyDigits(s: String?): String =
    s?.filter(Char::isDigit)?.take(11).orEmpty()

/** UI에서 입력받은 digits를 서버 전송용 하이픈 포맷으로 변환 */
fun hyphenatePhone(digitsRaw: String): String {
    val d = phoneOnlyDigits(digitsRaw)
    if (d.isEmpty()) return ""
    val breaks = computeBreaks(d)
    val sb = StringBuilder()
    for (i in d.indices) {
        sb.append(d[i])
        val pos = i + 1 // 1-based
        if (pos != d.length && breaks.contains(pos)) sb.append('-')
    }
    return sb.toString()
}
...
fun computeBreaks(d: String): List<Int> {
...
fun isoToDigits(iso: String?): String =
    iso?.replace("-", "")?.take(8).orEmpty()

/** "20101010" -> "2010-10-10" (8자리 완성일 때만 반환, 아니면 null) */
